...
2. `ssq_config.py` 管理策略默认值、参数校验和命令行选项，`ssq_modeling.py` 负责特征工程、模型训练和号码评分，`ssq_analyzer.py` 执行滚动回测和流程编排，`ssq_rules.py` 集中管理组合硬规则、软评分与单式组合选择，`ssq_reporting.py` 独立生成分析报告。
