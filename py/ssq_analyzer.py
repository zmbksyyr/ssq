...
# --- 参数保存和加载的辅助函数 ---
def save_params_to_json(params, filepath=PARAMS_JSON_PATH):
    """将最优参数保存到JSON文件。"""
    try:
...
def load_params_from_json(filepath=PARAMS_JSON_PATH):
    """从JSON文件加载参数。如果失败，返回None。"""
    try:
        with open(filepath, 'r', encoding='utf-8') as f:
            params = json.load(f)
        print(f"已成功从 {filepath} 加载固化参数。")
        return params
    except Exception as e:
        print(f"加载参数文件失败: {e}。")
        return None
...
def load_and_preprocess_data(filepath=CSV_PATH):
    try:
        df = pd.read_csv(filepath, header=0); df.columns = ['期号', '日期', '红球', '蓝球']
...
def feature_engineer(df):
    ...
    def count_consecutive_groups(nums):
        groups = 0; in_group = False
        for i in range(len(nums) - 1):
            if nums[i+1] - nums[i] == 1:
                if not in_group: groups += 1; in_group = True
            else: in_group = False
        return groups
...
def get_weighted_frequency(series, decay_factor):
    N = len(series); weights = np.array([decay_factor ** (N - i - 1) for i in range(N)])
    weighted_counts = {}
    for i, sublist in enumerate(series):
        for ball in sublist: weighted_counts[ball] = weighted_counts.get(ball, 0) + weights[i]
...
def run_strategy_and_get_scores(df_history, params, ml_models_red, ml_models_blue, feature_columns):
    ...
    for col in last_features.columns:
        if last_features[col].isnull().any(): last_features[col].fillna(df_history[col].mean(), inplace=True)
    ...
    for ball in range(1, 34):
        ...
        red_scores[ball] = (norm_freq * params['weight_freq'] + norm_omission * params['weight_omission'] + red_ml_probs[ball] * params['weight_ml'])
    hot_counts = pd.Series([b for sl in df_history.tail(params['hot_lookback'])['红球'] for b in sl]).value_counts()
    for ball in hot_counts[hot_counts >= params['hot_threshold']].index: red_scores[ball] *= params['hot_bonus']
    cold_numbers = set(range(1, 34)) - set([b for sl in df_history.tail(params['cold_lookback'])['红球'] for b in sl])
    for ball in cold_numbers: red_scores[ball] *= params['cold_bonus']
    for ball in df_history.iloc[-1]['红球']: red_scores[ball] *= params['repeat_bonus']
    ...
    for ball in range(1, 17):
        ...
        blue_scores[ball] = (norm_blue_freq * params['weight_blue_freq'] + blue_ml_probs[ball] * params['weight_blue_ml'])
    return red_scores, blue_scores

# --- 3. 组合生成与回测评估 (无变化) ---
def generate_recommendations(red_scores, blue_scores, num_single=10):
    sorted_reds = sorted(red_scores, key=red_scores.get, reverse=True)
    sorted_blues = sorted(blue_scores, key=blue_scores.get, reverse=True)
    duplex_reds, duplex_blues = sorted_reds[:7], sorted_blues[:7]
    core_balls, pool_balls = sorted_reds[:4], sorted_reds[4:12]
    single_combinations = []
    if len(pool_balls) >= 2:
        for tow_combo in combinations(pool_balls, 2):
            single_combinations.append(sorted(core_balls + list(tow_combo)))
            if len(single_combinations) >= num_single: break
...
def run_backtest(params, full_df, ml_models_red, ml_models_blue, backtest_range, prize_rules, cost_per_ticket, feature_columns):
    ...
    for i in range(start_index, end_index):
        ...
        hit_key = (red_hits, blue_hit); prize = prize_rules.get(hit_key, 0)
        if prize > 0: prize_counts[hit_key] += 1
...
if __name__ == '__main__':
    ...
    WINDOW_SIZE = 5
    FEATURE_COLUMNS = ['red_sum', 'red_span', 'odd_count', 'blue_lag1', 'red_zone_small', 'red_zone_medium', 'red_zone_large', 'red_big_count', 'red_prime_count', 'red_sum_tail', 'red_consecutive_groups', 'red_ac_value', 'red_tail_uniques', 'red_sum_lag1', 'odd_count_lag1', 'red_sum_ma5', 'odd_count_ma5', 'blue_ma5']

    print("正在加载数据..."); full_df = load_and_preprocess_data()
    if full_df is None: exit()
    print("正在进行特征工程..."); full_df = feature_engineer(full_df)
    
    # --- MODIFICATION: 简化优化决策逻辑 ---
    print("正在检查是否存在已固化的参数文件...")
    run_full_optimization = not os.path.exists(PARAMS_JSON_PATH)
    # ----------------------------------------
    ...
    for i in tqdm(range(1, 34), desc="训练红球模型"):
        ...
        if not X.empty: lgb_clf = lgb.LGBMClassifier(random_state=42, verbose=-1); lgb_clf.fit(X, y); ml_models_red[i] = lgb_clf
    for i in tqdm(range(1, 17), desc="训练蓝球模型"):
        ...
        if not X.empty: lgb_clf = lgb.LGBMClassifier(random_state=42, verbose=-1); lgb_clf.fit(X, y); ml_models_blue[i] = lgb_clf
        
    best_params, best_profit, best_prize_counts = {}, 0, {}
    backtest_start_index = max(1000, WINDOW_SIZE + 1)
    backtest_end_index = len(full_df) - 1

    # --- MODIFICATION: 根据新的决策逻辑执行 ---
    if not run_full_optimization:
        print("\n" + "="*50 + "\n【模式】: 跳过优化，加载固化参数\n" + "="*50)
        print(f"原因: 发现已存在的参数文件 {PARAMS_JSON_PATH}。")
        best_params = load_params_from_json()
        if best_params is None: # 如果加载失败，使用一套预设的默认值
            print("加载失败！将使用默认预设参数进行本次分析。请检查JSON文件或删除它以重新触发优化。")
            best_params = {'decay_factor': 0.995, 'weight_freq': 0.3, 'weight_omission': 0.4, 'weight_ml': 0.3, 'hot_lookback': 15, 'hot_threshold': 3, 'hot_bonus': 1.2, 'cold_lookback': 20, 'cold_bonus': 1.05, 'repeat_bonus': 1.05, 'weight_blue_freq': 0.5, 'weight_blue_ml': 0.5}

        print("\n正在为固化参数计算历史回测利润和中奖详情...")
        best_profit, best_prize_counts = run_backtest(best_params, full_df, ml_models_red, ml_models_blue, (backtest_start_index, backtest_end_index), PRIZE_RULES, COST_PER_TICKET, FEATURE_COLUMNS)
        print("计算完成。")
    else:
        print("\n" + "="*50 + "\n【模式】: 完整优化，并行搜索最优参数\n" + "="*50)
        print(f"原因: 未找到参数文件 {PARAMS_JSON_PATH}，执行首次优化。")
        
        param_space = {'decay_factor': [0.995, 0.999], 'weight_freq': [0.3], 'weight_omission': [0.4], 'weight_ml': [0.3], 'hot_lookback': [10, 15], 'hot_threshold': [2, 3], 'hot_bonus': [1.1, 1.2], 'cold_lookback': [20, 30], 'cold_bonus': [1.05, 1.1], 'repeat_bonus': [1.05, 1.15], 'weight_blue_freq': [0.4, 0.5, 0.6], 'weight_blue_ml': [0.6, 0.5, 0.4]}
        ...
        results = Parallel(n_jobs=-1)(delayed(run_backtest)(p, full_df, ml_models_red, ml_models_blue, (backtest_start_index, backtest_end_index), PRIZE_RULES, COST_PER_TICKET, FEATURE_COLUMNS) for p in tqdm(all_param_combinations, desc="优化进度"))
    ...
    report_lines = []; report_lines.append("="*60); report_lines.append("          双色球策略分析与推荐报告 (固化参数版)"); report_lines.append("="*60)
    latest_issue = str(full_df.iloc[-1]['期号'])
    try: target_issue = int(latest_issue) + 1
    except ValueError: target_issue = f"{latest_issue}_Next"
    report_lines.append("\n--- 0. 报告元数据 ---"); report_lines.append(f"Data_Basis_Issue: {latest_issue}"); report_lines.append(f"Prediction_Target_Issue: {target_issue}")
    report_lines.append(f"\n报告生成时间: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    report_lines.append("\n--- 1. 策略参数 ---")
    mode_desc = "执行首次优化并固化" if run_full_optimization else "加载已固化的参数"
    report_lines.append(f"模式: {mode_desc}")
    for key, val in best_params.items(): report_lines.append(f"  - {key:<20}: {val}")
    ...
    for (red, blue), count in best_prize_counts.items():
        if count > 0: prize_name = PRIZE_NAMES.get((red, blue)); aggregated_counts[prize_name] += count
    if sum(aggregated_counts.values()) == 0: report_lines.append("  - 未中任何奖项。")
    else:
        for prize_name in ["一等奖", "二等奖", "三等奖", "四等奖", "五等奖", "六等奖"]:
            if (count := aggregated_counts.get(prize_name, 0)) > 0: report_lines.append(f"  - {prize_name:<5}: {count} 次")
    report_lines.append("\n--- 2. 推荐组合 ---")
    top_blue = duplex_combo[1][0] if duplex_combo[1] else "N/A"
    report_lines.append("\n【单式推荐 (10组)】")
    if single_combos:
        for i, combo in enumerate(single_combos, 1): report_lines.append(f"  组合 {i:>2}: 红球 {str(combo):<24} 蓝球 [{top_blue:02d}]")
    else: report_lines.append("  - 未能生成足够的单式组合。")
    report_lines.append("\n【7+7 复式推荐 (1组)】"); report_lines.append(f"  红球: {duplex_combo[0]}"); report_lines.append(f"  蓝球: {duplex_combo[1]}")
    report_lines.append("\n" + "="*60 + "\n报告结束。祝您好运！\n" + "="*60)
    ...
    try:
        ...
        with open(filepath, 'w', encoding='utf-8') as f: f.write(final_report_string)
        print(f"\n\n报告已成功保存到文件: {filepath}")
    except Exception as e:
        print(f"\n\n写入报告文件失败: {e}")
